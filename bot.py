...
async def get_last_info(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
	...
	try:
		...
		us.add_check(1)
		await context.bot.send_message(chat_id=chat_id, text=msg.get_message("error_check", get_language(context)), parse_mode=ParseMode.HTML)
	return CHECKING
...
async def name_for_list(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
	...
	if list_name in context.chat_data["watchlists"]:
		await context.bot.send_message(chat_id=chat_id, text=msg.get_message("overwrite_set_list", get_language(context)), parse_mode=ParseMode.HTML)
	else:
		context.chat_data["watchlists"].append(list_name)
	context.chat_data[context.chat_data["active_key"]] = []
	await context.bot.send_message(chat_id=chat_id, text=msg.get_message("set_list_3", get_language(context)), parse_mode=ParseMode.HTML)
	return SETLIST_BCBA
...
async def add_bcba_symbol_to_list(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
	...
	if not symbol == "OK":
		...
		if is_in:
			...
			await context.bot.send_message(chat_id=chat_id, text=msg.get_selection(get_language(context)), parse_mode=ParseMode.HTML)
		else:
		...
		await context.bot.send_message(chat_id=chat_id, text=msg.get_message("set_list_4", get_language(context)), parse_mode=ParseMode.HTML)
...
async def trigger_eraselist(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
	...
	if not context.chat_data == None:
		...
		await context.bot.send_message(chat_id=chat_id, text=msg.get_message("erase_list_1", get_language(context)), reply_markup=reply, parse_mode=ParseMode.HTML)
		us.add_list(2)
		return ERASE_LIST
	else:
		await context.bot.send_message(chat_id=chat_id, text=msg.get_message("error_list", get_language(context)), parse_mode=ParseMode.HTML)
		us.add_list(3)
		return ConversationHandler.END
...
async def confirm_eraselist(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
	...
	if context.chat_data["erase_list"] == the_list:
		...
		await context.bot.send_message(chat_id=chat_id, text=msg.get_message("error_erase_list", get_language(context)), parse_mode=ParseMode.HTML)
		us.add_list(3)
		return ConversationHandler.END
...
async def get_last_dolar(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
	chat_id = update.effective_chat.id
	try:
		await context.bot.send_message(chat_id=chat_id, text=msg.get_message("start_dolar", get_language(context)), parse_mode=ParseMode.HTML)
		await context.bot.send_message(chat_id=chat_id, text=msg.get_analysis_emoji(), parse_mode=ParseMode.HTML)
		message = msg.build_dolar_message(mk.dolar_ar_data(), get_language(context))
		await context.bot.send_message(chat_id=chat_id, text=message, parse_mode=ParseMode.HTML)
		us.add_dolar(0)
	except:
...
async def user_watchlists(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
	...
	if "watchlists" in context.chat_data:
		if len(context.chat_data["watchlists"]) > 0:
			keyboard = user_lists_keyboard(context.chat_data["watchlists"])
			reply = InlineKeyboardMarkup(keyboard)
			await context.bot.send_message(chat_id=chat_id, text=msg.get_message("start_list", get_language(context)), reply_markup=reply, parse_mode=ParseMode.HTML)
		else:
			await context.bot.send_message(chat_id=chat_id, text=msg.get_message("error_list", get_language(context)), parse_mode=ParseMode.HTML)
			us.add_list(3)
	else:
...
async def report_error(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
	...
	await context.bot.send_message(chat_id=chat_id, text=msg.get_emoji("envelope"), parse_mode=ParseMode.HTML)
	return ConversationHandler.END
...
async def print_help(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
	...
	await context.bot.send_message(chat_id=chat_id, text=msg.get_emoji("magnifying_glass"), parse_mode=ParseMode.HTML)
...
async def select_language(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
	...
	reply = InlineKeyboardMarkup(keyboard)
	await context.bot.send_message(chat_id=chat_id, text=msg.get_message("language_1", get_language(context)), reply_markup=reply, parse_mode=ParseMode.HTML)
...
async def set_language(update: Update, context: ContextTypes.DEFAULT_TYPE, query) -> None:
	...
	if query == "l_1":
		...
		await context.bot.send_message(chat_id=chat_id, text=msg.get_emoji("mate"), parse_mode=ParseMode.HTML)
...
async def default_button_click(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
	...
	if query.data.startswith("wl"):
		if len(context.chat_data[query.data]) > 0:
			...
			await context.bot.send_message(chat_id=chat_id, text=msg.get_message("error_list_2", get_language(context)), parse_mode=ParseMode.HTML)
			await context.bot.send_message(chat_id=chat_id, text=msg.get_apology(get_language(context)), parse_mode=ParseMode.HTML)
