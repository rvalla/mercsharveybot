...
async def get_about(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
	...
	try:
		...
		us.add_about(1)
		await context.bot.send_message(chat_id=id, text=msg.get_message("error_about", get_language(id)), parse_mode=ParseMode.HTML)
	return ABOUT
...
def build_general_conversation_handler():
	...
	handler = ConversationHandler(
		entry_points=[CommandHandler("about", trigger_about), CommandHandler("bcba", trigger_check),
					CommandHandler("world", trigger_check), CommandHandler("error", trigger_error_submit)],
		states={
...
def main() -> None:
	...
	app.add_handler(CommandHandler("list", get_user_list), group=2)
