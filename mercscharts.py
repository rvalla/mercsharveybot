...
def lines_chart(w, h, resolution, data, error):
    ...
    for r in range(len(data)):
        plt.fill_between(range(len(data[0])), error[r*2], error[r*2+1], facecolor=colors[r], alpha=0.3, zorder=1)
        plt.plot(data[r], linewidth=2.75, color=colors[r], zorder=2)
    plt.grid(which='both', axis='both')
    plt.grid(True, "major", "y", ls="-", lw=0.8, c="dimgrey", alpha=0.9)
    plt.grid(True, "major", "x", ls="-", lw=0.8, c="dimgrey", alpha=0.9)
    plt.minorticks_on()
    if len(data[0]) > 11:
        plt.xticks(range(0,len(data[0]),3))
    else:
        plt.xticks(range(0,len(data[0]),1))
